...
// Returns the page/database ID and its type (logKeyPageID or "database_id").
	...
	for i := range maxDepth {
		...
		case logKeyPageID:
			c.logger.DebugContext(ctx, "resolved block to page",
				logKeyBlockID, blockID,
				logKeyPageID, block.Parent.PageID,
				"depth", i+1)
			return normalizePageID(block.Parent.PageID), logKeyPageID, nil
		case "database_id":
...
func (c *Crawler) savePageFromNotion(ctx context.Context, page *notion.Page, folder string, isRoot bool) error {
	...
	if err != nil && strings.Contains(err.Error(), "is a database, not a page") {
		c.logger.DebugContext(ctx, "detected database, saving as database", logKeyPageID, pageID)
		...
		syntheticPage := &notion.Page{
			...
			Properties: notion.Properties{
				notion.PropTypeTitle: {Type: notion.PropTypeTitle, Title: database.Title},
			},
		}
	...
	parentID := c.resolveParentID(ctx, pageID, logKeyPageID, page.Parent)
